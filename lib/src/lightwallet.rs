...
impl Default for SendProgress {
    fn default() -> Self {
        Self { id: 0, is_send_in_progress: false, progress: 0, total: 0, last_error: None, last_txid: None }
    }
}

impl SendProgress {
    fn new(id: u32) -> Self {
        SendProgress { id, ..Default::default() }
    }
}
...
impl<P: consensus::Parameters + Send + Sync + 'static> LightWallet<P> {
    ...
    pub async fn in_memory_keys<'this>(
        ...
        tokio::sync::RwLockReadGuard::try_map(keys, |keys| {
            match keys {
    ...
    pub async fn in_memory_keys_mut<'this>(
        ...
        tokio::sync::RwLockWriteGuard::try_map(keys, |keys| {
            match keys {
                Keystores::Memory(keys) => Some(keys),
                _ => None,
            }
    ...
    pub async fn read<R: Read>(
        ...
        if version <= 8 {
            ...
            let spendable_keys = keys
                .get_all_spendable_ivks()
                .await
                .collect();
    ...
    async fn select_notes_and_utxos(
        ...
        for anchor_offset in &self.config.anchor_offset {
            ...
            let notes = candidate_notes
                ...
                .collect::<Vec<_>>();
            let sapling_value_selected = notes
                .iter()
        ...
        Fut: Future<Output = Result<String, String>>,
    {
        ...
        if selected_value < (total_value + fees).unwrap() {
            let e = format!(
                "Insufficient verified funds. Have {} zats, need {} zats + {} zats in fees. NOTE: funds need at least \
        ...
        println!("{}: Adding {} notes and {} utxos", now() - start_time, notes.len(), utxos.len());
        ...
        // Set up a channel to receieve updates on the progress of building the
        // transaction
...
mod test {
    ...
    async fn z_t_note_selection() {
        ...
        let (notes, utxos, selected, _fees) = lc
            .wallet
            .select_notes_and_utxos(amt, false, false, 0, 0)
            .await;
        assert!(selected >= amt);
        ...
        let (notes, utxos, _selected, _fees) = lc
            .wallet
            .select_notes_and_utxos(amt, false, false, 0, 0)
            .await;
        ...
        let (notes, utxos, selected, _fees) = lc
            .wallet
            .select_notes_and_utxos(amt, false, true, 0, 0)
            .await;
        assert!(selected >= amt);
        assert_eq!(notes.len(), 1);
        assert_eq!(notes[0].note.value, value);
        ...
        let (notes, utxos, selected, _fees) = lc
            .wallet
            .select_notes_and_utxos(amt, true, true, 0, 0)
            .await;
    ...
    async fn multi_z_note_selection() {
        ...
        // Reset the anchor offsets
        lc.wallet.config.anchor_offset = [9, 4, 2, 1, 0];
        let (notes, utxos, selected, fees) = lc
            .wallet
            .select_notes_and_utxos(amt, false, false, 0, 0)
            .await;
        assert!(selected >= amt);
