...
pub fn main() {
    ...
    if seed.is_some() && maybe_birthday.is_none() {
        ...
        eprintln!(
            "This should be the block height where the wallet was created. If you don't remember the block height, \
             you can pass '--birthday 0' to scan from the start of the blockchain."
        );
        return;
    }
